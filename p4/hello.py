...
def login():
    ...
    historial =  anadirPagina("login.html")
...
def conectarBD():
    try:
        ...
        print("No se ha podido conectar la base de datos: %s") % e
    return client['test']

@app.route('/restaurantes.html', methods=['POST','GET'])
...
def restaurantes():
    ...
    if request.method == 'POST':
        ...
        rest = db.restaurants.find({"name" : {"$regex" : nombre}})

    else:
        rest = db.restaurants.find()

    if 'username' in session:
        user=session['username']#prácticas
        historial =  anadirPagina("restaurantes.html")
        return render_template("restaurantes.html", rest = rest, tam = rest.count(), user = user, historial = historial)
    else :
        return render_template("restaurantes.html", rest = rest, tam = rest.count())
